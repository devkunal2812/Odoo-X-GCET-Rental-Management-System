...
async function testRelationships() {
  ...
  try {
    ...
    // Test 2: Coupon → SaleOrder (potential relationship)
    ...
    if (order) {
      ...
      console.log(`      - Vendor: ${order.vendor.user.email}`);
      console.log(`      - Coupon: ${order.coupon?.code || 'None'}`);
      console.log(`      - Discount: $${order.discount}`);
      console.log(`      - Total: $${order.totalAmount}`);
    } else {
      console.log('   ⚠️  No orders found (expected in fresh database)');
    }

    // Test 4: Rental Period Configs
    console.log('\n4️⃣ Testing Rental Period Configs');
